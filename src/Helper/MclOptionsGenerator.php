<?php
namespace Concrete\Package\ThemeAnitya\Src\Helper;

defined('C5_EXECUTE') or die(_("Access Denied."));

use \Concrete\Package\ThemeAnitya\Src\Helper\MclBaseOptionsGenerator;
use Concrete\Package\ThemeAnitya\Src\Models\MclOptions;
use Core;
use Loader;

/**
 * @package Anitya theme Options
 * @category Helper
...
class MclOptionsGenerator {
	...
	function __construct ($options = null, $pID = null, $url = null, $url_view = null, $saved_options = null) {
		...
		if ($options) : // Option n'est pas rempli quand il est appelle par loader::helper
			...
			$this->pID = $pID;
			$this->generator = new MclBaseOptionsGenerator($pID);
			$this->url = $url;
			$this->options = $options;
			$this->saved_options = $saved_options ? $saved_options : MclOptions::get_options_from_preset_ID($pID);
